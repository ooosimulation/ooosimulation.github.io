...
export default class GameBall {
    ...
    // 🌟 [변경]: 대시 돌진 타이머가 활성화(this.dashTimer > 0)되어 있는 동안에도 완전 무적 상태가 되도록 게터 메서드를 확장 보정합니다.
    get isInvincible() {
        return this._isInvincible || (this.dashTimer > 0);
    }

    set isInvincible(newVal) {
        this._isInvincible = newVal;
    }
    ...
    draw(ctx) {
        ...
        // 🌟 [유지]: 플레이어 공 위에 대시 3칸 스택 및 충전 상태 상시 드로잉 게이지바 UI
        if (this.team === 'player') {
            ctx.save();
